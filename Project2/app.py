...
def callback():
    ...
    # Store refresh token server-side for the rotation test.
    # NEVER print the actual refresh token.
...
def test_refresh_rotation():
    ...
    if not refresh_token:
        return """
        <h2>Refresh Rotation Test</h2>
        <p>ERROR: No refresh token in session.</p>
        <p>Log out and log in again with offline_access enabled.</p>
        """
    ...
    # IMPORTANT:
    # Do NOT replace the session's refresh token yet.
    # We intentionally keep the OLD token for replay testing.
    ...
    if second.status_code == 200:
        ...
        return """
        <h2>Refresh Rotation Test</h2>
        <p style="color:red;">SECURITY TEST FAILED</p>
